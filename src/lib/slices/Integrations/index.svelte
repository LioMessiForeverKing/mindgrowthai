<script lang="ts">
	...
	import PhCircleWavyWarningLight from '~icons/ph/circle-wavy-warning-light'
	import SolarAlignBottomBold from '~icons/solar/align-bottom-bold'
	import CarbonAperture from '~icons/carbon/aperture'
	import CarbonScisControlTower from '~icons/carbon/scis-control-tower'
	import PhAlarmLight from '~icons/ph/alarm-light'
	...
	const icons = {
		learning: IconDigitalOcean,
		knowledge: PhAlarmLight,
		accessibility: PhCircleWavyWarningLight,
		growth: SolarAlignBottomBold,
		education: CarbonAperture,
		happiness: CarbonScisControlTower
	};
	...
	<div class="relative">
		...
		<div class="mt-20 flex flex-col items-center md:flex-row">
			{#each slice.primary.repeatable_zone as item, index}
				{#if item.icon}
					{#if index === Math.floor(slice.primary.repeatable_zone.length / 2)}
						<StylizedLogoMark />
					...
					{#if index !== slice.primary.repeatable_zone.length - 1}
						<div
							class={clsx(
								'signal-line',
								index >= Math.floor(slice.primary.repeatable_zone.length / 2) ? 'rotate-180' : 'rotate-0'
							)}
						></div>
					{/if}
				{/if}
